...
| Minimal redesign: no tags bar / frequency badges; More collapse; state label; Z/X/C shortcuts (R owned by Anki); listening resolver wiring | `tests/test_templates.py` §§6b–6c |
...
| Mature Word Mode: const, exact current-card retrieval (`guiCurrentCard`→`cardsInfo`) before heuristic fallback, no `note:` clause, stub/timeout/fallback rules, anti-flash gate, word-mode CSS, Z/X/C-only shortcuts, finish.sh push-before-release ordering | `tests/test_templates.py` §§8b, §9 |
...
| Listening resolver behavior (**Policy B**): `#listening` + usable audio → listening; `#listening` + no audio → safe sentence fallback; legacy audio-only (Sentence/Word Audio) → listening; no-tag gloss cards / Frequency-legacy / no-audio → sentence front. Exactly ONE visible sound button (a pure `#listening` audio card renders both tag and classic views in markup; the resolver keeps only the active one) and no sound button when audio/sentence is missing | `tests/test_front_modes.py` — extracts the real resolver from the front template and expands the **real template markup** in headless Chrome across 10 field harnesses; skipped gracefully when Chrome is absent |
| Mature-mode duplicate-card safety: duplicate Expression disambiguated by Sentence then cloze-body; ambiguous duplicates fail safely (never candidate 0); exact current-card interval is authoritative over any duplicate; expression-mismatch leaks no interval | `tests/test_mature_content.py` — extracts `resolveMatureByContent` + `resolveMatureExactCard` verbatim and runs them in Node against stubbed AnkiConnect; skipped gracefully when Node is absent |
